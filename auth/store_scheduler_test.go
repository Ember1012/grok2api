...
func TestNeedsUsageProbeSkipsRateLimitedCooldown(t *testing.T) {
	...
	if acc.NeedsUsageProbe(10 * time.Minute) {
		t.Fatal("NeedsUsageProbe should return false during rate_limited cooldown")
	}
...
func TestNeedsUsageProbeSkipsWhenUsageFresh(t *testing.T) {
	now := time.Now()
	acc := &Account{
		AccessToken:         "token",
		Status:              StatusReady,
	...
	if acc.NeedsUsageProbe(10 * time.Minute) {
		t.Fatal("NeedsUsageProbe should return false when usage snapshot is fresh")
	}
}
