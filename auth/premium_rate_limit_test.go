...
func TestPremium5hRateLimitedSkipsUsageProbe(t *testing.T) {
	acc := newPremium5hTestAccount("pro", time.Now().Add(30*time.Minute))

	// premium 5h 限流期间不发探针（避免 /responses 加重限流）。
	if acc.NeedsUsageProbe(10 * time.Minute) {
		t.Fatal("NeedsUsageProbe() = true, want false during premium 5h limit")
	}
	if !acc.InLimitedState() {
		t.Fatal("InLimitedState() = false, want true during premium 5h limit")
	}
}
