...
```text
ghcr.io/Ember1012/grok2api:latest
```

如需锁定首个稳定版本，可改用：

```text
ghcr.io/Ember1012/grok2api:1.0.0
```
...
之后 `.github/workflows/render-deploy.yml` 会在 `Build Docker Image` 工作流成功后自动请求该 Hook，让 Render 重新拉取 `ghcr.io/Ember1012/grok2api:latest` 并部署。
...
services:
  codex2api:
    image: ghcr.io/Ember1012/grok2api:1.0.0
